...
import { ProductDTO, Query } from '../types';
const db = require('../db');

class FindController {
  /*async getFind(req: Request, res: Response){
        const findstring = req.query.findstring;
        const findAuthorName = await db.query("SELECT id, name, type_id, (SELECT name FROM autor WHERE autor.id = product.autor_id) AS author_name (SELECT name FROM manufacturer WHERE manufacturer.id = product.manufacturer_id) AS manufacturer, (SELECT surname FROM autor WHERE autor.id = product.autor_id) AS author_surname (SELECT name FROM publisher WHERE publisher.id = product.publisher_id) AS publisher, price, image, count, badge_novelty, badge_discount, badge_sales_leder AS badge_sales_leader, (SELECT discount_amount FROM discount WHERE discount.id = product.discount_id) AS discount, (SELECT SUM(evaluation)/COUNT(*) FROM review WHERE review.product_id = product.id) AS review FROM product WHERE autor_id IN (SELECT id FROM autor WHERE LOWER(name) LIKE LOWER('%"+findstring+"%'))");
  ...
  async getFind(req: Request, res: Response) {
    ...
    let tx_query_resp =
      "SELECT DISTINCT * FROM ((SELECT id, name, type_id, autor_id AS author_id , publisher_id, manufacturer_id,(SELECT name FROM autor WHERE autor.id = product.autor_id) AS author_name (SELECT name FROM manufacturer WHERE manufacturer.id = product.manufacturer_id) AS manufacturer, (SELECT surname FROM autor WHERE autor.id = product.autor_id) AS author_surname (SELECT name FROM publisher WHERE publisher.id = product.publisher_id) AS publisher, price, year_of_publication, image, count, badge_novelty, badge_discount, badge_sales_leder AS badge_sales_leader, (SELECT discount_amount FROM discount WHERE discount.id = product.discount_id) AS discount, (SELECT SUM(evaluation)/COUNT(*) FROM review WHERE review.product_id = product.id) AS review FROM product WHERE autor_id IN (SELECT id FROM autor WHERE LOWER(name) LIKE LOWER('%" +
      findstring +
    ...
    const find: Query<ProductDTO[]> = await db.query(tx_query_resp);

    res.json(find.rows);
  }

  async getInfoFind(req: Request, res: Response) {
    const findstring = req.query.findstring;
    ...
    let tx_query_resp =
      "SELECT DISTINCT * FROM ((SELECT id, autor_id AS author_id , publisher_id, manufacturer_id, binding_id, type_id, (SELECT name FROM autor WHERE autor.id = product.autor_id) AS author_name (SELECT name FROM binding WHERE binding.id = product.binding_id) AS binding, (SELECT name FROM manufacturer WHERE manufacturer.id = product.manufacturer_id) AS manufacturer, (SELECT surname FROM autor WHERE autor.id = product.autor_id) AS author_surname (SELECT name FROM publisher WHERE publisher.id = product.publisher_id) AS publisher, badge_novelty, badge_discount, badge_sales_leder AS badge_sales_leader, price, count, (SELECT discount_amount FROM discount WHERE discount.id = product.discount_id) AS discount, (SELECT SUM(evaluation)/COUNT(*) FROM review WHERE review.product_id = product.id) AS review FROM product WHERE autor_id IN (SELECT id FROM autor WHERE LOWER(name) LIKE LOWER('%" +
      findstring +
      "%'))) UNION (SELECT id, autor_id AS author_id , publisher_id, manufacturer_id, binding_id, type_id, (SELECT name FROM autor WHERE autor.id = product.autor_id) AS author_name (SELECT name FROM binding WHERE binding.id = product.binding_id) AS binding, (SELECT name FROM manufacturer WHERE manufacturer.id = product.manufacturer_id) AS manufacturer, (SELECT surname FROM autor WHERE autor.id = product.autor_id) AS author_surname (SELECT name FROM publisher WHERE publisher.id = product.publisher_id) AS publisher, badge_novelty, badge_discount, badge_sales_leder AS badge_sales_leader, price, count, (SELECT discount_amount FROM discount WHERE discount.id = product.discount_id) AS discount, (SELECT SUM(evaluation)/COUNT(*) FROM review WHERE review.product_id = product.id) AS review FROM product WHERE autor_id IN (SELECT id FROM autor WHERE LOWER(surname) LIKE LOWER('%" +
      findstring +
      "%'))) UNION (SELECT id, autor_id AS author_id , publisher_id, manufacturer_id, binding_id, type_id, (SELECT name FROM autor WHERE autor.id = product.autor_id) AS author_name(SELECT name FROM binding WHERE binding.id = product.binding_id) AS binding,  (SELECT name FROM manufacturer WHERE manufacturer.id = product.manufacturer_id) AS manufacturer, (SELECT surname FROM autor WHERE autor.id = product.autor_id) AS author_surname (SELECT name FROM publisher WHERE publisher.id = product.publisher_id) AS publisher, badge_novelty, badge_discount, badge_sales_leder AS badge_sales_leader, price, count, (SELECT discount_amount FROM discount WHERE discount.id = product.discount_id) AS discount, (SELECT SUM(evaluation)/COUNT(*) FROM review WHERE review.product_id = product.id) AS review FROM product WHERE autor_id IN (SELECT id FROM autor WHERE LOWER(CONCAT(name, ' ', surname)) LIKE LOWER('%" +
      findstring +
      "%'))) UNION (SELECT id, autor_id AS author_id , publisher_id, manufacturer_id, binding_id, type_id, (SELECT name FROM autor WHERE autor.id = product.autor_id) AS author_name (SELECT name FROM binding WHERE binding.id = product.binding_id) AS binding, (SELECT name FROM manufacturer WHERE manufacturer.id = product.manufacturer_id) AS manufacturer, (SELECT surname FROM autor WHERE autor.id = product.autor_id) AS author_surname (SELECT name FROM publisher WHERE publisher.id = product.publisher_id) AS publisher, badge_novelty, badge_discount, badge_sales_leder AS badge_sales_leader, price, count, (SELECT discount_amount FROM discount WHERE discount.id = product.discount_id) AS discount, (SELECT SUM(evaluation)/COUNT(*) FROM review WHERE review.product_id = product.id) AS review FROM product WHERE autor_id IN (SELECT id FROM autor WHERE LOWER(CONCAT(surname, ' ', name)) LIKE LOWER('%" +
    ...
    const count_query =
      'SELECT COUNT(*) FROM (' + tx_query_resp + tx_query + ') AS f_results';
