...
class profileController {
    ...
    async getShopping_cartCheck(req, res){
        const id = req.params.id;
        const cart = await db.query("SELECT id FROM product WHERE id IN (SELECT product_id FROM shopping_cart WHERE account_id = $1)", [id]);
        res.json(cart.rows);
    }
    async getShopping_cartCount(req, res){
        const id = req.params.id;
        const cart = await db.query("SELECT COUNT(id) FROM product WHERE id IN (SELECT product_id FROM shopping_cart WHERE account_id = $1)", [id]);
        res.json(cart.rows);
    }
    async getOrders(req, res){
        const id = req.params.id;
        const orders = await db.query("SELECT order_date, product_id, order_code, (SELECT image FROM product WHERE product.id = product_id) AS product_image, (SELECT price FROM product WHERE product.id = product_id) AS price FROM order_list WHERE account_id = $1", [id]);
        res.json(orders.rows);
    }
    async getOrder(req, res){
        const code = req.params.code;
        const profile = req.query.profile;
        const order = await db.query("SELECT product_id, (SELECT image FROM product WHERE product.id = product_id) AS product_image, (SELECT name FROM product WHERE product.id = product_id) AS name, (SELECT price FROM product WHERE product.id = product_id) AS price, (SELECT name FROM autor WHERE autor.id = (SELECT autor_id FROM product WHERE product.id = product_id)) AS autor_name, (SELECT surname FROM autor WHERE autor.id = (SELECT autor_id FROM product WHERE product.id = product_id)) AS autor_surname, (SELECT name FROM manufacturer WHERE manufacturer.id = (SELECT manufacturer_id FROM product WHERE product.id = product_id)) AS manufacturer FROM order_list WHERE order_code = $1 AND account_id = $2", [code, profile]);
        res.json(order.rows);
    }
