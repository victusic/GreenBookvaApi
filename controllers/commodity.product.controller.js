...
class commodityProductController {
    ...
    async getProductList(req, res){
        ...
        let tx_query_resp = "SELECT id, name, autor_id, (SELECT name FROM autor WHERE autor.id = product.autor_id) AS autor_name, (SELECT surname FROM autor WHERE autor.id = product.autor_id) AS autor_surname, manufacturer_id, (SELECT name FROM manufacturer WHERE manufacturer.id = product.manufacturer_id) AS manufacturer, publisher_id, (SELECT name FROM publisher WHERE publisher.id = product.publisher_id) AS publisher, price, image, count, badge_novelty, badge_discount, badge_sales_leder, (SELECT discount_amount FROM discount WHERE discount.id = product.discount_id) AS discount, (SELECT SUM(evaluation)/COUNT(*) FROM review WHERE review.product_id = product.id) AS review FROM product WHERE";
    ...
    async getProductOneImages(req, res){
        const id = req.params.id;
        const product = await db.query("SELECT image FROM product_image WHERE id IN (SELECT image_id FROM image_product WHERE product_id = $1)", [id]);
        res.json(product.rows);
    }

    async getProductOneImagesList(req, res){
        const id = req.params.id;
        const product = await db.query("SELECT image, 1 as sort_order FROM product WHERE id = $1 UNION SELECT image, 2 as sort_order FROM product_image WHERE id IN (SELECT image_id FROM image_product WHERE product_id = $1) ORDER BY sort_order", [id]);
        res.json(product.rows);
    }

    async getProductOneReviews(req, res){
        const id = req.params.id;
        const product = await db.query("SELECT id, account_id, header, review_text, evaluation, (SELECT name FROM account WHERE account.id = review.account_id) AS user_name FROM review WHERE product_id = $1", [id]);
        res.json(product.rows);
    }

    async getProductTypes(req, res){
        const id = req.query.type;
        const productCategories = await db.query("SELECT id FROM category WHERE id IN (SELECT category_id FROM type_category_subcategory WHERE type_id = $1)", [id]);
